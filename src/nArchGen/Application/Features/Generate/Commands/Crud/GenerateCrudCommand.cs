...
public class GenerateCrudCommand : IStreamRequest<GeneratedCrudResponse>
{
    ...
    public class GenerateCrudCommandHandler : IStreamRequestHandler<GenerateCrudCommand, GeneratedCrudResponse>
    {
        ...
        )
        {
            ...
            response.CurrentStatusMessage = "Generating Data layer codes...";
            yield return response;
            newFilePaths.AddRange(await generateDataCodes(request.ProjectPath, request.CrudTemplateData));
            response.LastOperationMessage = "Data layer codes have been generated.";

            response.CurrentStatusMessage = "Generating Business layer codes...";
            yield return response;
            newFilePaths.AddRange(await generateBusinessCodes(request.ProjectPath, request.CrudTemplateData));
            response.LastOperationMessage = "Business layer codes have been generated.";
        ...
        private async Task<string> injectEntityToContext(string projectPath, CrudTemplateData crudTemplateData)
        {
            string contextFilePath = PlatformHelper.SecuredPathJoin(
                projectPath,
                "Pars.RM.RetailApi.MobileStoreApi.Data",
                "EF",
        ...
        private async Task<ICollection<string>> generatePersistenceCodes(string projectPath, CrudTemplateData crudTemplateData)
        {
            string templateDir = PlatformHelper.SecuredPathJoin(
                ...
                Templates.Paths.Crud,
                "Folders",
                "Pars.RM.RetailApi.MobileStoreApi.Domain"
            );
            return await generateFolderCodes(
                templateDir,
                outputDir: PlatformHelper.SecuredPathJoin(projectPath, "Pars.RM.RetailApi.MobileStoreApi.Domain"),
                crudTemplateData
            );
        }
        private async Task<ICollection<string>> generateDataCodes(string projectPath, CrudTemplateData crudTemplateData)
        {
            string templateDir = PlatformHelper.SecuredPathJoin(
                DirectoryHelper.AssemblyDirectory,
                Templates.Paths.Crud,
            ...
            return await generateFolderCodes(
                templateDir,
                outputDir: PlatformHelper.SecuredPathJoin(projectPath, "Pars.RM.RetailApi.MobileStoreApi.Data"),
                crudTemplateData
            );
        }
        private async Task<ICollection<string>> generateBusinessCodes(string projectPath, CrudTemplateData crudTemplateData)
        {
            string templateDir = PlatformHelper.SecuredPathJoin(
                DirectoryHelper.AssemblyDirectory,
                Templates.Paths.Crud,
                "Folders",
                "Pars.RM.RetailApi.MobileStoreApi.Business"
            );
            return await generateFolderCodes(
                templateDir,
                outputDir: PlatformHelper.SecuredPathJoin(projectPath, "Pars.RM.RetailApi.MobileStoreApi.Business"),
                crudTemplateData
            );
        }
